...
DROP TABLE outliers_vru_time;
CREATE TABLE outliers_vru_time LIKE calls;
ALTER TABLE outliers_vru_time ADD COLUMN metodo TINYINT;
...
WHERE vru_time > (SELECT AVG(vru_time) FROM calls) + (3 * (SELECT stddev(vru_time) FROM calls)) 
      ...
      OR vru_time < 0; #1.691 outliers (0.38%)
							
#Insertar outliers segun BoxPlot. MAX='0.2666666666666666' MIN='2.7755575615628914e-17'
INSERT INTO outliers_vru_time
SELECT *, 2
FROM calls
WHERE vru_time > 0.2666666666666666 OR vru_time < 2.7755575615628914e-17; #41220 Outliers (9.27%)
...
SELECT * FROM outliers_vru_time; #75.435
...
JOIN  outliers_vru_time o ON a.cod = o.cod
SET a.out_sig = 0
WHERE o.metodo=1;
